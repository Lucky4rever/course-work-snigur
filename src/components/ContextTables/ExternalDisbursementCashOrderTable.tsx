...
import { getExternalDisbursementCashOrders } from "../../helpers/Neo4jDriver";
...
class ExternalDisbursementCashOrderTable extends ContextTable {
  state: StateProps = { 
    rows: [],
    isLoaded: false 
  };
  ...
  ContextTableRow: React.FC<DisbursementCashOrder> = ({ date, sum, basis, sender, receiver, destinationCode, analyticalAccountingCode, correspondingAccount, EDRPOU }) => {
    return (
      <>
        ...
        <div>{correspondingAccount.length === 0 ? "-" : destinationCode}</div>
        <div>{EDRPOU}</div>
  ...
  AllItems = async () => {
    const items = await getExternalDisbursementCashOrders();
...
export default ExternalDisbursementCashOrderTable;